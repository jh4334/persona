...
class MyClassrooms:
    ...
    def get(self, user_id: str, cid: str) -> dict | None: ...

    def create(self, user_id: str, data: dict, cid: str) -> None: ...

    def delete(self, user_id: str, cid: str) -> bool: ...
...
class SupabaseClassrooms(MyClassrooms):
    ...
    def __init__(self, url: str, key: str, table: str = "classrooms") -> None:
        import httpx
    ...
    def list(self, user_id: str) -> list[dict]:
        r = self._check(self._client.get(
            ...
            f"&order=updated_at.desc&limit={MAX_PER_USER}"), "목록 조회")
        out: list[dict] = []
    ...
    def get(self, user_id: str, cid: str) -> dict | None:
        r = self._check(self._client.get(
            f"{self.endpoint}?select=data&id=eq.{cid}&user_id=eq.{user_id}&limit=1"), "조회")
        rows = r.json() or []
...
class ClassroomLibrary:
    ...
    def __init__(self, root: Path, disk_dir: Path) -> None:
        ...
        if cfg:
            url, key, _ = cfg
            try:
                self.mine: MyClassrooms = SupabaseClassrooms(url, key)
                log.info("내 학급 저장소: Supabase (classrooms 테이블)")
    ...
    def list(self, user_id: str | None) -> list[dict]:
        ...
        try:
            out = self.mine.list(user_id or "local") + out   # 내 학급을 위에
        except Exception as exc:
            # 내 학급을 못 읽어도 샘플로 수업은 시작할 수 있어야 한다
            log.warning("내 학급 목록 조회 실패: %s", exc)
        return out

    def resolve(self, user_id: str | None, cid: str) -> tuple[Classroom, dict]:
        ...
        if cid.startswith("sample:"):
            ...
            return got
        data = self.mine.get(user_id or "local", cid)
        if data is None:
            raise ClassroomError("학급을 찾을 수 없습니다. 목록을 새로고침해 주세요.")
        return parse_classroom(data, "학급"), data

    def create(self, user_id: str | None, text: str) -> dict:
    ...
    def delete(self, user_id: str | None, cid: str) -> None:
        if cid.startswith("sample:"):
            raise ClassroomError("샘플 학급은 지울 수 없습니다.")
        if not self.mine.delete(user_id or "local", cid):
