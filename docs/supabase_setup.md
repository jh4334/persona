...
셋업 화면의 **[+ 내 학급 만들기]** 를 누르면 탭이 둘 나온다.

**폼으로 만들기 (기본)** — JSON을 몰라도 된다.

1. 학급 이름·학년을 넣는다
2. **[인원 한 번에 만들기]** 로 성취 수준별 인원(상 2, 중상 3, 중 4 …)을 지정하면
   그만큼 학생 초안이 생긴다. 한 명씩 **[+ 학생 추가]** 해도 된다
3. 학생을 눌러 펼치고 이름·사전 지식·흥미 등을 채운다. 비슷한 학생은
   **[이 학생 복제]** 가 빠르다
4. 세부 속성이 필요하면 9개 그룹(인지·학습 능력, 동기·정서 …)을 펼쳐 고른다.
   **전부 선택 사항**이라 비워 둬도 수업은 돌아간다. 채운 개수가 머리글에 표시된다
5. [저장]

**JSON 붙여넣기** — 이미 학급 파일이 있으면 그대로 붙여넣거나 파일을 고른다.

어느 쪽이든 형식이 틀리면 어디가 문제인지 한국어로 짚어 준다 (`학생 S03에
name(이름)이 없습니다` 처럼). 학급 이름이나 학생 이름이 비었거나 겹치는 것은
서버에 보내기 전에 폼에서 먼저 잡는다. 저장하면 목록 맨 위 "내 학급" 묶음에
들어가고 바로 선택된다.

JSON 형식은 `personas/class_6_3.json`과 같다. 최소한 이 정도면 된다:
...
# 4단계 — 수업 기록 (지난 수업 다시 보기)

끝난 수업의 리포트·전사는 디스크(`reports/stage/`)에만 남았다. 두 가지가
아쉬웠다: 컨테이너를 재배포하면 사라지고, 교사가 리포트 화면을 벗어나면
파일을 직접 뒤지지 않는 한 다시 찾을 수 없었다.

이제 셋업 화면 아래 **[지난 수업 기록]** 에 끝난 수업이 쌓인다. 누르면 그때
리포트가 그대로 다시 열리고, 전사도 함께 내려받을 수 있다.

## 설정

`db/schema.sql`을 다시 Run 하면 `reports` 테이블이 추가된다. 환경변수는 그대로다.

**디스크 파일 쓰기는 그대로 유지된다.** 서버에 남는 `.md`·`.transcript.json`은
그 자체로 쓸모가 있어서 없애지 않았다. 옆에 `.meta.json`이 하나 늘었을 뿐이다
(목록에 필요한 학급명·단원·턴 수 등).

| 설정 | 목록·열람을 어디서 읽나 | 디스크 파일 |
|---|---|---|
| Supabase 있음 | Supabase (원격 장애 시 디스크로 대체) | 그대로 남음 |
| 없음 | 디스크 `.meta.json` | 그대로 남음 |

원격 저장이 실패해도 수업은 끝나고 리포트는 화면에 나온다 — 경고 로그만 남는다.

## 보관 기간

세션 스냅샷과 달리 수업 기록은 **자동으로 지워지지 않는다.** 교사가 직접
목록에서 [✕]로 지운다. 무료 티어 용량이 걱정되면 오래된 기록을 정리하는
크론을 추가할 수 있다:

```sql
select cron.schedule('purge-old-reports', '0 19 * * 0',
  $$delete from public.reports
     where created_at_epoch < extract(epoch from now()) - 365 * 86400$$);
```
